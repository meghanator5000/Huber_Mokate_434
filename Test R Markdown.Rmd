...
master_set <- master_set %>% 
  mutate(`Net Productivity Change` = (`Net Productivity` - lag(`Net Productivity`, default = first(`Net Productivity`)))) 
...
for (i in seq(nrow(master_set))) {
  if (i == 1) {
    ...
    master_set$increase_act[i] <- ((master_set$`Minimum Wage`[i] - master_set$`Minimum Wage`[i - 1])/master_set$`Minimum Wage`[i])}
}
...
ggplot() +
  geom_path(data = master_set, aes(x = Year, y = `Increase Amount`, color = `Increase Type`)) 
+
    geom_abline(aes(intercept = mean(master_set$incr_diff), slope = 0), color = "black", size = 1)  
```
...
for (i in seq(nrow(master_set))) {
  if (i == 1) {
    ...
    master_set$prod_adj[i] <- (master_set$prod_adj[i - 1]*master_set$`Net Productivity Change`[i]) + master_set$prod_adj[i-1]}
}


# Calculate expected minimum wage based off inflation AND productivity

master_set$net_impact <- master_set$ann_inflat + master_set$`Net Productivity Change`
