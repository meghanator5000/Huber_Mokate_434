...
When comparing changes in raw minimum wage to GDP Projected minimum wage, we see that none of the decades come close to measuring up to the adjusted level. However, the 1950s through 1970s are more closely aligned with these adjusted values. The 1980s and 2010s have very large gaps between these values. 
...
We calculated two variables: GDP Projected Minimum Wage and Minimum Wage adjusted for Purchasing Power.

The GDP Projected Minimum Wage calculates minimum wage in 2018 dollars adjusted for GDP, and shows an expected adjusted minimum wage value of $39.45 in 2018. 
...
write_csv(inflation_dataset, "Inflation_Dataset.csv")
# inflation_dataset <- read_csv("Inflation_Dataset.csv")
...
### Decades GDP Projected: Change in minimum wage values by decade for raw minimum wage and GDP Projected minimum wage
```{r}
# this portion will look at changes in raw minimum wage and GDP Projected minimum wage from the first year in a decade to the last year in the decade for each decade starting with the 1950s and going through the 2010s, with 2018 as the end value for this decade (minimum wage has not changed since 2018)
decades_GDPadj <- main_dataset %>%
...
decades_GDPadj$decade_change <- currency(decades_GDPadj$decade_change, digits = 2L)
decades_GDPadj$beg_decade_wage <- currency(decades_GDPadj$beg_decade_wage, digits = 2L)
decades_GDPadj$beg_decade_wage_adj <- currency(decades_GDPadj$beg_decade_wage_adj, digits = 2L)
decades_GDPadj$decade_change_adj <- currency(decades_GDPadj$decade_change_adj, digits = 2L)

raw_wage_footnote <- "Raw minimum wage in the first year of the decade"
adj_wage_footnote <- "GDP Projected Minimum Wage in the first year of the decade"

decades_footnotes = c(raw_wage_footnote,
              adj_wage_footnote)

decades_GDPadj_table <- decades_GDPadj %>%  
  kable(col.names = c("Decade",
                      "Raw Decade Change",
                      "GDP Projected Decade Change",
                      "Raw Minimum Wage[note]",
                      "GDP Projected Minimum Wage[note]"), 
      align = "cccc",
      caption = "Changes in Raw and GDP Projected Minimum Wage by Decade",
...
decades_GDPadj_table
```

```{r}
decades_GDPadj_graph <- decades_GDPadj %>%
  ...
  scale_fill_manual(values = c("decade_change" = "darkseagreen4", "decade_change_adj" = "royalblue4"),
                    labels = c("Raw Decade Change", "GDP Projected Decade Change" )) + # change colors to match our theme
  labs(title = "Decade Change: Raw and \nGDP Projected Minimum Wage", # title graph
       y = "Minimum Wage Change", # label y axis
       x = "Decade", # label x axis
       caption = "Different changes look at raw minimum wage change \nand GDP Projected dollars",
       fill = "Minimum Wage Value Change") + 
  wage_theme +
  theme(plot.title = element_text(hjust = 0.5)) # center the title

decades_GDPadj_graph # view the graph
```

#### Decades GDP Projected Interpretation: 
...
This chart shows the side by side comparison of how minimum wage actually changed decade to decade (raw) versus how it should have changed to keep up with GDP Projected levels. While none of the decades come close to measuring up to the adjusted level, the 1950s through 1970s raw minimum wage changes show similar trends in terms of direction and comparative increases to the adjusted value. However, this pattern is thrown off in 1980s, with a significant decline offsetting the GDP increase. Hopes for recovery from this decline are seen through the 1990s and 2000s, but are squashed in the 2010s when the decade change falls to 0 - a decrease that is much more significant than the GDP adjusted decrease in the decade.

### Decades Purchasing Power: Change in minimum wage values by decade for raw minimum wage and purchasing power of minimum wage
```{r}
...
decades_PurchPower <- main_dataset %>%
...
# make the rows currency values for the table
decades_PurchPower$decade_change <- currency(decades_PurchPower$decade_change, digits = 2L)
decades_PurchPower$beg_decade_wage <- currency(decades_PurchPower$beg_decade_wage, digits = 2L)
decades_PurchPower$beg_decade_wage_pp <- currency(decades_PurchPower$beg_decade_wage_pp, digits = 2L)
decades_PurchPower$decade_change_pp <- currency(decades_PurchPower$decade_change_pp, digits = 2L)
...
decades_PurchPower_table <- decades_PurchPower %>%  
  kable(col.names = c("Decade",
                      "Raw Decade Change",
...
decades_PurchPower_table

```

```{r}
decades_PurchPower_graph <- decades_PurchPower %>%
  mutate(decade_change = as.numeric(decade_change), # change to numeric so that we are able to pivot
  ...
  labs(title = "Decade Change: Raw Minimum \nWage and its Purchasing Power", # title graph
       y = "Minimum Wage Change", # label y axis
       x = "Decade", # label x axis
...
decades_PurchPower_graph # view the graph
```

#### Decades Purchasing Power Interpretation: 
...
### Decades All: Change in minimum wage values by decade for raw minimum wage, purchasing power of minimum wage, and GDP Projected minimum wage
```{r}
decades_all <- main_dataset %>%
...
# make the rows currency values for the table
decades_all$decade_change <- currency(decades_all$decade_change, digits = 2L)
decades_all$beg_decade_wage <- currency(decades_all$beg_decade_wage, digits = 2L)
decades_all$beg_decade_wage_pp <- currency(decades_all$beg_decade_wage_pp, digits = 2L)
decades_all$decade_change_pp <- currency(decades_all$decade_change_pp, digits = 2L)
decades_all$beg_decade_wage_adj <- currency(decades_all$beg_decade_wage_adj, digits = 2L)
decades_all$decade_change_adj <- currency(decades_all$decade_change_adj, digits = 2L)
...
decades_table_2 <- decades_all %>%  
  kable(col.names = c("Decade",
                      ...
                      "GDP Projected Minimum Wage[note]"), 
      align = "cccc",
      caption = "Changes in Raw, GDP Projected, and Purchasing Power of Minimum Wage by Decade",
      format.args = list(big.mark = ","),
      digits = c(0, 2, 2)) %>% # specify decimal points
  kable_paper(font_size = 12,       
              html_font = "Georgia",
...
decades_graph_2 <- decades_all %>%
  ...
  scale_fill_manual(values = c("decade_change" = "darkseagreen4", 
                    ...
                    labels = c("Raw Decade Change", "GDP Projected Decade Change", "Purchasing Power Decade Change")) + # change colors to match our theme and ensure legend labels are helpful
  labs(title = "Decade Change of Minimum Wage", # title graph
       y = "Minimum Wage Change", # label y axis
       x = "Decade", # label x axis
       caption = "Different changes look at raw minimum wage change, \nGDP Projected change and purchasing power change",
       fill = "Minimum Wage Value Change") + 
  wage_theme +
  theme(plot.title = element_text(hjust = 0.5)) # center the title
...
#### Decades All Interpretation: 
Following our analysis derived from Decades and decades_PurchPower, we decided to plot all three findings on one plot to demonstrate our findings in a comprehensive explanatory manner. The significance in size of the GDP Projected Decade Change highlight the inability of the raw decade change to keep pace with the changes decade to decade. Further, the negative values for purchasing power and in the 1980s and 2010s stand out as indicating a shift in the treatment of minimum wage to be a bit more conservative. Including all three bars also provides useful context. For instance, if we only had raw decade change, the graph would suggest that the increase in the 2000s was more significant than in all other decades, since the raw value increase is the highest ($2.00). However, when we compare this to the GDP Projected Decade Change bar, we see that it fails to come close to this value, and earlier decades are much closer than the 2000s.

Through examining this graph, we can clearly see how the relationship between raw minimum wage, GDP Projected minimum wage, and purchasing power of minimum wage changed decade to decade. 
...
wage_values_graph <- 
  ...
  labs(title = "$ Disparity in Actual and Adjusted \nMinimum Wage over Time", # title graph
...
### GDP Projected Versus Raw Value:

The graph shows the steady shallow slope of the raw value of minimum wage over time, ending in a value of \$7.25 in 2018. 
On the other hand, the GDP Projected line, which calculates minimum wage in 2018 dollars adjusted for GDP, shows a much steeper slope. The slope particularly appears to begin its sharp increase in the late 1970s and really takes off around 1981 (the first year of the Reagan administration), ending with a value of $39.45. The gap between these lines demonstrates the amount that wage has failed to increase proportionately with GDP, or net productivity. In terms of informing policy, a GDP adjusted minimum wage of almost \$40 makes the "Fight for 15" appear much less radical.
...
3. decades_GDPadj = includes column for proportional differences in raw minimum wage and GDP Projected minimum wage from start to end of decade
4. decades_GDPadj_table = table for decades
5. decades_GDPadj_graph = graphed findings for decades
6. decades_PurchPower = includes column for proportional differences in raw minimum wage and purchasing power of minimum wage from start to end of decade
7. decades_PurchPower_table = table for decades_PurchPower
7. decades_PurchPower_graph = graphed findings for decades_PurchPower
8. decades_all = includes column for proportional differences in raw minimum wage, GDP Projected minimum wage, and purchasing power of minimum wage from start to end of decade
9. decades_all_table = table for decades_all
10. decades_all_graph = graphed findings for decades_ with proportions_2
...
#### Decades
We perform exploratory analysis in decades_GDPadj and decades_PurchPower before performing final, explanatory analysis in the decades_all table and graph.

To create the tables and graphs for decades, we did some calculations to find the change in minimum wage through the decade and add three columns for each of these values (raw minimum wage, GDP Projected minimum wage, and purchasing power of minimum wage). We also kept the value for the starting minimum wage for each of these types in the table, to provide context for the change.
