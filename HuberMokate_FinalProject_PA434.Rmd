...
Click here to see exploratory decades table:
```{r}
decades <- main_dataset %>%
  select(year, raw_min_wage) %>%
  mutate(year = as.character(year)) %>%
  filter(str_detect(year, "0$") | str_detect(year, "9$") & year != "1949") %>%
  mutate(decade_change = raw_min_wage - lag(raw_min_wage, default = first(raw_min_wage))) %>%
  filter(str_detect(year, "9$")) %>%
  mutate(decade = str_replace(year, "9$", "0s")) %>%
  select(decade, decade_change, raw_min_wage) 
...
decades_table <- decades %>%  
  kable(col.names = c("Decade",
      ...
      align = "cc",
      caption = "Raw Minimum Wage Change Each Decade",
  ...
  add_footnote("Raw minimum wage in the final year of the decade")
  
decades_table
```
Click to view the exploratory decades graph creation:
```{r}
...
decades__prop_graph <- decades_prop %>%
  ...
  labs(title = "Proportional Change in Raw Minimum Wage from \nEnd of Decade to Start of Decade", # title graph
       ...
       x = "Decade", # label x axis
       caption = "Changes shown as a proportion of raw minimum wage the final year of the decade") + 
  wage_theme +
  theme(plot.title = element_text(hjust = 0.5)) # center the title
...
```
Click here to view the table preparation:
...
```
Click here to view the table creation:
```{r create-table, include = TRUE}
table <- 
  table_dataset %>% 
