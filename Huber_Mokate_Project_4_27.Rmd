...
library(knitr)
library(blscrapeR) # to request inflation calculations from BLS' API and complete inflation-related calculations 
```
...
todo: jane. I think it might be a 20 year limit, not ten year limit.
Might be worth cleaning up, but this isn't a big deal. Also might be
valuable to get a data access key from BLS

Useful Q & A about what CPI is and how to interpret:
https://www.bls.gov/cpi/questions-and-answers.htm#Question_15
```{r get-inflation-data}
...
annual_cpi_tb <- 
  inflation_dataset %>% 
  group_by(year) %>% 
  summarize(avg_annual_cpi = mean(value)) %>% 
  mutate()
...
# Step 2: Create a column for annual inflation that uses the formula from BLS

# Step 3: Convert Minimum Wage dollar values to 2018

# The API will determine the percent increase from a dollar in
# 1948 (our starting year). We then use that percentage to determine
# what our starting value should be for our minimum wage datasets.

# Only run this when need to; is an API request
#converted_1948_min_wage <- inflation_adjust(1948)

converted_1948_min_wage_2018 <- 
  converted_1948_min_wage %>% 
  filter(year == "2018")

starting_minwage_value_2018_dollars <- (.4) * (10.4)
