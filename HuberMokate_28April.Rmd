...
# Fill in missing values for raw minimum wage, then filter to 1948, which is the
# year we will use as a base. 1948 was chosen because that was the first year
# that the US government measured Productivity.

main_dataset <- 
  main_dataset %>% 
  arrange(desc(desc(year))) %>% 
...
# The productivity multiplier is a variable representing the yearly increase in
# productivity. The net productivity value is a percent increase comparing the 
# GDP value to 1948, which is the first year this measurement was collected by 
# the US government. In order to determine what the minimum wage value would be 
# if it rose every year according to the increase in productivity, we will 
# multiply the minimum wage of the previous year by this productivity multiplier.
...
# 1 dollar today = 10 cents in 1948 money in 1948 had 90% less purchasing power

# Applying purchasing power
main_dataset$minwage_purchasingpower <- 
  (cpi_for_2018 / main_dataset$annual_avg_cpi) * main_dataset$raw_min_wage
...
table_dataset <- 
  left_join(min_wage_dataset, main_dataset, by = "year") %>% 
  filter(!is.na(annual_avg_cpi)) %>% 
  select(year,
         raw_min_wage.x,
         min_wage_2018_adjusted,
         min_wage_prod_adjusted,
...
table <- 
  ...
  kable(col.names = c("Year[note]", 
                    ...
                    "Income Change[note]"),
      align = "cccccccc",
      caption = "Average Income and # Children by Year Gender Since 2000[note]",
