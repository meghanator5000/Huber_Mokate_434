...
main_dataset$perc_inc_prod <- vector("numeric", nrow(main_dataset))

for (i in 1:nrow(main_dataset)) {
  
  perc_inc <- ifelse(i==1, 
                     1,
                     (main_dataset$net_productivity[i] /
                        main_dataset$net_productivity[i-1]))
  
  main_dataset$perc_inc_prod[i] <- perc_inc
...
for (i in 1:nrow(main_dataset)) {
  ...
  inflation_rate <- 1 + (((cpi_for_2018 - cpi_for_year) / cpi_for_year) /100)
...
for (i in 1:nrow(main_dataset)) {
  ...
  main_dataset$min_wage_prod_adjusted[i] <- 
    main_dataset$min_wage_2018_adjusted[i] * main_dataset$perc_inc_prod[i]
